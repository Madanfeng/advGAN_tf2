...
def dataset(istrain):
    ...
    l = len(images_path)
    targets = np.full(l, cfg.TARGET, dtype=np.int32)
    ...
    # ds = image_label_target_path_ds.apply(
    #     tf.data.experimental.shuffle_and_repeat(buffer_size=l))
    ds = image_label_target_path_ds.repeat()
...
if __name__ == '__main__':
    ...
    #     # next(it)
    #     for images, labels, targets, paths in ds.take(steps_per_epoch+1):
    #         # if j == 0:
    #         print(paths[0])
    #         j += 1
    #     print(j)
    ...
    ds, l = dataset(istrain=False)

    t1 = time.time()
    i = 0
    path_list = []
    for image, lab, target, path in ds.take(int(l/32)):
        i += 1
        print(i)
        for p in path:
